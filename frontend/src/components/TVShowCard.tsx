...
export const TVShowCard = ({ show, onScrape, isSelected = false, onToggleSelect }: Props) => {
      ...
      <div className="aspect-[2/3] w-full relative">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-4">
          ...
          <div className="translate-y-4 group-hover:translate-y-0 transition-transform duration-300 delay-75">
            <h3 className="font-bold text-white text-sm line-clamp-1 mb-1">{show.title}</h3>
            <div className="flex items-center gap-2 text-[10px] font-semibold text-zinc-300">
              <span className="text-green-400">{Math.round((show.tmdb_rating || 0) * 10)}% Match</span>
              <span>{show.year}</span>
            </div>
          </div>
        </div>
